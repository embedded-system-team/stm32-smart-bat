...
- `firmware/Core/Inc/wifi_credentials.h` is not tracked
- `firmware/build/` is not tracked
- Godot `.godot/` cache files are not tracked
- README, templates, source code, and required configuration files are committed

Example release tag:

```bash
git tag -a v0.1.0 -m "Smart Bat playable milestone"
git push origin v0.1.0
```

After pushing the tag, create a GitHub Release from the `v0.1.0` tag.

## Current Milestone

- STM32 swing detection and event output are working
- UDP bridge to Godot is working
- Basic Godot 3D swing gameplay is working
- Future work can improve calibration, hit feel, game progression, and release artifacts
